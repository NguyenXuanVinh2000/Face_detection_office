...
class VideoThreading(object):
    def __init__(self, src=0):
        # Create a VideoCapture object
        self.frame = None
        self.capture = cv2.VideoCapture(src)
        # self.capture = FileVideoStream(src).start()
        ...
        while True:
            if self.capture.isOpened():
                ...
                if self.frames % 3 == 0:
                    ...
                    box, landmark, confidence = api_detector(self.frame)
                    ...
                    if len(box) > 0:
                        for i in range(0, len(confidence)):
                            ...
                            nimg = face_preprocess.preprocess(self.frame, bbox, landmarks, image_size='112,112')
                            ...
                            cv2.putText(self.frame, text, (bbox[0], y), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
                            cv2.rectangle(self.frame, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 0, 0), 2)
                            FPS = 3 / (time.time() - start_time)
                            fps = str(round(FPS))
                            cv2.putText(self.frame, fps, (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                else:
                    for tracker, text in zip(self.trackers, self.texts):
                        ...
                        cv2.putText(self.frame, text, (startX, startY - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.45,
                                    (0, 0, 255), 2)

    def show_frame(self, device):
        # Display frames in main program
        if self.status:
            self.frame = imutils.resize(self.frame, width=700)
            cv2.imshow(device, self.frame)
