...
cap = cv2.VideoCapture("rtsp://admin:ervai!@#123@172.16.14.27/")
cap2 = cv2.VideoCapture("rtsp://admin:ervai!@#123@172.16.14.21/")
_, frame = cap2.read()
...
while True:
    _, frame1 = cap.read()
    _, frame2 = cap2.read()
    frame = np.concatenate((cv2.resize(frame1, (700, 500)), cv2.resize(frame2, (700, 500))), axis=1)
    q.put(frame)
    if not _:
        break
